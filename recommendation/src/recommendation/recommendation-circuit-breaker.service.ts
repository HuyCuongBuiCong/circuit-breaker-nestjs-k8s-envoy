...
export class RecommendationCircuitBreakerService {
  ...
  constructor(private readonly recommendationService: RecommendationService) {
    this.circuitBreaker = new opossum(
      this.recommendationService.fetchRecommendations.bind(
    ...
    this.circuitBreaker.fallback(() =>
      this.recommendationService.defaultRecommendations(),
    );
    this.circuitBreaker.on('open', () => this.logCircuitStatus('open'));
    this.circuitBreaker.on('halfOpen', () =>
      this.logCircuitStatus('half open'),
    );
  ...
  private logCircuitStatus(status: string) {
    ...
    console.log(
      `Circuit breaker stats: ${JSON.stringify(this.circuitBreaker.stats)}`,
    );
  }
}
